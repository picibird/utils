# VSCode

## Plugins

- [antfu.browse-lite](https://github.com/antfu/vscode-browse-lite)
- [vscode-jupyter](https://github.com/microsoft/vscode-jupyter)
- [codetour](https://github.com/microsoft/codetour)
- [vscode-nodebook](https://github.com/microsoft/vscode-nodebook)
- [nuxt-vscode-extension](https://github.com/AllanOricil/nuxt-vscode-extension)

## Languages

- [typescript](https://code.visualstudio.com/docs/languages/typescript)

## Multi-root Workspaces

> You can work with multiple project folders in Visual Studio Code with [multi-root workspaces](https://code.visualstudio.com/docs/editor/multi-root-workspaces).

## Debug

- [VS Code Recipes](https://github.com/microsoft/vscode-recipes)
- [JavaScript Debugging Recipes](https://code.visualstudio.com/docs/nodejs/debugging-recipes)


## Devcontainer

- [Developing inside a container on a remote Docker host](https://code.visualstudio.com/docs/remote/containers-advanced#_developing-inside-a-container-on-a-remote-docker-host)
- [devcontainer.json reference](https://code.visualstudio.com/docs/remote/devcontainerjson-reference)
- [Advanced Container Configuration](https://code.visualstudio.com/docs/remote/containers-advanced)

## Remote

- [VS Code Remote Development](https://code.visualstudio.com/docs/remote/remote-overview)
- [Remote development in Containers](https://code.visualstudio.com/docs/remote/containers-tutorial)
- [Remote Development Tips and Tricks](https://code.visualstudio.com/docs/remote/troubleshooting)
- [Your development environment](https://code.visualstudio.com/docs/containers/choosing-dev-environment)
- [Debug containerized apps](https://code.visualstudio.com/docs/containers/debug-common)
- [Node.js in a container](https://code.visualstudio.com/docs/containers/quickstart-node)

